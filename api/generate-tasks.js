...
const buildCharacterNgrams = (value, size = 2) => {
...
const isSameCategory = (left, right) => inferTaskCategory(left) === inferTaskCategory(right);

const toSimilaritySignature = (task) => {
...
const calcNgramOverlap = (leftText, rightText) => {
  const left = buildCharacterNgrams(leftText);
  const right = buildCharacterNgrams(rightText);
  return calcTokenOverlap(left, right);
};

const isSimilarTask = (a, b) => {
  const sigA = toSimilaritySignature(a);
  const sigB = toSimilaritySignature(b);
  if (!sigA.combined || !sigB.combined) {
    return false;
  }
  if (sigA.normalizedTitle === sigB.normalizedTitle || sigA.combined === sigB.combined) {
    return true;
  }
  if (sigA.combined.includes(sigB.normalizedTitle) || sigB.combined.includes(sigA.normalizedTitle)) {
    return true;
  }
  if (calcTokenOverlap(sigA.titleTokens, sigB.titleTokens) >= 0.8) {
    return true;
  }
  return calcTokenOverlap(sigA.combinedTokens, sigB.combinedTokens) >= 0.72;
};

const isSemanticallyTooClose = (candidate, existing) => {
...
const toComparableTask = (task, index) => ({
...
const getRecentCompletedCategories = (existingTasks) => existingTasks
...
const filterRecentlyCompletedCategory = (candidates, existingTasks) => {
  const recentCategories = getRecentCompletedCategories(existingTasks);
  if (recentCategories.length === 0) {
    return { preferred: candidates, deferred: [] };
  }
...
const rankCandidatesByCategoryDiversity = (candidates) => {
...
const filterDuplicateCandidates = (candidates, existingTasks) => {
  ...
  for (const candidate of candidates) {
    if (existingTasks.some((task) => isSemanticallyTooClose(candidate, task))) {
      continue;
    }
    if (unique.some((task) => isSemanticallyTooClose(candidate, task))) {
      continue;
    }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return sendJson(res, 200, { tasks: finalTasks, generationStoppedReason });
  } catch (error) {
