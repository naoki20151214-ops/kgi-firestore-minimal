...
const canGenerateKgiNow = () => {
  ...
  const isInvestmentGenre = (wizardState.genreClassification?.primaryGenre || "").includes("投資");
  if (isInvestmentGenre) {
    const resolved = new Set(getResolvedAmbiguityIds());
    const hasCore = ["success_metric", "aggregation_unit", "target_market"].every((id) => resolved.has(id));
    return hasDeadline && hasSuccessState && hasCore && answeredCount >= 3;
  }
...
const generateInterviewOutcome = async () => {
  try {
    ...
    const data = await response.json().catch(() => null);
    if (!response.ok || !data?.aiKgiSourceData || !data?.kgiStatement) throw new Error("interview_outcome_api_error");
    wizardState.genreClassification = data.genreClassification || wizardState.genreClassification;
    wizardState.aiKgiSourceData = ensureRequiredSourceDataFields(data.aiKgiSourceData);
    wizardState.kgiStatement = data.kgiStatement;
    wizardState.kgiSuccessCriteria = Array.isArray(data.kgiSuccessCriteria) ? data.kgiSuccessCriteria : [];
    wizardState.gapAnalysis = data.gapAnalysis || null;
    wizardState.kpiDrafts = Array.isArray(data.kpiDrafts) ? data.kpiDrafts : [];
    wizardState.clarifiedSuccessState = data.clarifiedSuccessState || wizardState.aiKgiSourceData.currentKgiScope;
    wizardState.nextKgiSuggestion = wizardState.aiKgiSourceData.nextKgiSuggestion || "";
    wizardState.interviewNotes = Array.isArray(data.interviewNotes) ? data.interviewNotes : [];
    return;
  } catch (_error) {
    buildAiKgiSourceData();
    buildKgiResultFromSourceData();
    wizardState.gapAnalysis = {
      alreadyDone: ["初回入力と追加質問の回答が揃っている"],
      notDoneYet: ["現実との差分の詳細確認"],
      firstBigMountain: "最低公開ラインを期限内で満たす",
      gapToCloseForCurrentKgi: ["達成判定条件ごとの未充足項目を埋める"]
    };
    wizardState.kpiDrafts = ["達成判定条件ごとの進捗を週次で可視化する", "最低公開ラインの未達項目をゼロにする"];
  }
};
...
startDeepDiveButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const genre = await classifyGenre();
    ...
    if (wizardState.followUpQuestionHistory.length === 0 || canGenerateKgiNow()) {
      ...
      await generateInterviewOutcome();
      await updateCreationSession();
...
sourceDataApproveButton?.addEventListener("click", async () => {
  ...
  if (!wizardState.kgiStatement || !wizardState.kgiSuccessCriteria.length) {
    buildKgiResultFromSourceData();
  }
  await updateCreationSession();
  renderProposal();
