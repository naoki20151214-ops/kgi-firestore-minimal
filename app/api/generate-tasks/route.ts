...
const filterDuplicateCandidates = (candidates: NormalizedTask[], existingTasks: SimilarityComparableTask[]) => {
  ...
  for (const candidate of candidates) {
    const duplicateWithExisting = existingTasks.some((task) => isSimilarTask(candidate, task));
    if (duplicateWithExisting) {
      continue;
    }
    const duplicateInCandidates = unique.some((task) => isSimilarTask(candidate, task));
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingComparableTasks = existingTasksForTargetKpi.map((task) => ({
      title: isNonEmptyString((task as any)?.title) ? (task as any).title.trim() : "",
      description: isNonEmptyString((task as any)?.description) ? (task as any).description.trim() : ""
    }));
